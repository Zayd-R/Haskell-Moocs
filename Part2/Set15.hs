...
sumTwoMaybes m1 m2 = myLiftA2 (+) m1 m2
...
statements s1 s2 = [ w1 ++ " is " ++ w2 | w1 <- s1, w2 <- s2] ++ [ w1 ++ " is not " ++ w2 | w1 <- s1, w2 <- s2]
...
calculator op numStr = case op of
    "negate" -> negate <$> readMaybe numStr
    "double" -> (* 2) <$> readMaybe numStr
    _        -> Nothing
------------------------------------------------------------------------------
...
validateDiv a b = liftA2 safeDiv (pure a) myNum
            where myNum = check (b > 0) "Division by zero!" b
                  safeDiv x y = (div) x y
...
validateAddress streetName streetNumber postCode = liftA3  Address validSl validSr validP
                                                where validSl = check (length streetName <= 20) "Invalid street name" streetName
                                                      validSr = check (all (isDigit) streetNumber) "Invalid street number" streetNumber
                                                      validP  = check ( all (isDigit) postCode && length postCode == 5) "Invalid postcode" postCode
...
twoPersons :: Applicative f =>
  ...
  -> f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = (++) <$> (iHateThisCourse <$> name1 <*> age1 <*> employed1) <*> (iHateThisCourse <$> name2 <*> age2 <*> employed2)





iHateThisCourse :: String -> Int -> Bool -> [Person]
iHateThisCourse s i b = [Person s i b]
------------------------------------------------------------------------------
...
normalizePhone s =
  let stripped = removeSpaces s
      lengthCheck = check (length stripped <= 10) "Too long" stripped
      digitCheck = traverse (\c -> check (isDigit c) ("Invalid character: " ++ [c]) c) stripped
  in liftA2 (const id) lengthCheck digitCheck

removeSpaces [] = ""
removeSpaces (s:ss) = if isSpace s then removeSpaces ss else s : removeSpaces ss
------------------------------------------------------------------------------
...
checkNum n = check (isJust (readMaybe n :: Maybe Int)) ("Invalid number: " ++ n) (Number (read n :: Int))
checkVar n = check (all (isAlpha) n && length n == 1) ("Invalid variable: " ++ n) (Variable (head n))

parseExpression :: String -> Validation Expression
parseExpression s = if length (words s) /= 3 then invalid ("Invalid expression: " ++ s) else liftA3 myExpress checkNum1 checkOp checkNum2
                  where myExpress num1 op num2 = if op == "+" then Plus num1 num2  else Minus num1 num2
                        checkNum1 = checkNum (words s !! 0 ) <|> checkVar (words s !! 0 )
                        checkNum2 = checkNum (words s !! 2 ) <|> checkVar (words s !! 2 )
                        checkOp = check (words s !! 1 == "+" || words s !! 1 == "-") ("Unknown operator: " ++ words s !! 1) (words s !! 1)
...
f <#> x = myLiftA2 doSmth f x
        where doSmth f a = f a
...
tryAll f [] = empty
tryAll f (x:rest ) = try f x <|> (tryAll f rest)
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap f (Both res) = Both ( fmap (fmap f) res )
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  liftA2 f (Both res1) (Both res2) = Both (liftA2 (liftA2 f) res1 res2)