...
isSorted xs =  sorted xs === sorted (sort xs)
...
sumIsLength input output = foldl (\acc (_,x) -> acc + x) 0 output === length input
...
inputInOutput input output = forAll (elements input) (\x -> any (\(ele,_) -> ele == x) output )
...
outputInInput input output = forAll (elements output) (\(ele,num) -> isIt ele num input 0)

isIt elem num [] acc     = acc == num 
isIt elem num (x:xs) acc = if x == elem then isIt elem num xs (acc + 1) else isIt elem num xs acc 
...
frequenciesProp freq (NonEmpty input) =  conjoin [sumIsLength input b, inputInOutput input b, outputInInput input b] where b = freq input
