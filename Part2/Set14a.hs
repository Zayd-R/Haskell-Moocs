...
greetText t = if T.length t > 15 then T.concat [T.pack "Hello, ",T.take 15 t,T.pack "...!"] else T.concat [T.pack "Hello, ",t, T.pack "!"]
...
shout :: T.Text -> T.Text
shout t =    T.pack  ( take (T.length t) (shout' (words(T.unpack t)) 0))

shout' [] index = ""
shout' (word:words) index = if even index then map (toUpper) word  ++ " " ++ shout' words (index + 1) else word ++ " " ++ shout' words (index + 1)
...
longestRepeat t = longestRepeat' (T.unpack t) ' ' 0  0

longestRepeat' []  lastChar longest acc = max longest acc 
longestRepeat' (c:s) lastChar longest acc = if c /= lastChar  then if longest > acc then longestRepeat' s c 1 longest else longestRepeat' s c 1 acc 
                                                              else  longestRepeat' s c (longest + 1) acc
...
takeStrict makeLifeHard t =  TL.toStrict (TL.take  makeLifeHard t)
...
xorChecksum bs = case B.uncons bs of 
                    Nothing -> 0 
...
pingpong bs = let lazyBs = BL.fromStrict bs 
                in BL.append lazyBs (pingpong (B.reverse bs))  
