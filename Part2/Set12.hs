...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode a ll) = LNode (f a) (fmap f ll)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode a aa two) = TwoNode (f a) (f aa) (fmap f two)
...
count inital = foldr (\x acc -> if x == inital then acc + 1 else acc)  0
...
inBoth xs ys = inBoth' (toList xs) (toList ys) []

inBoth' [] ys acc = acc
inBoth' xs [] acc = acc
inBoth' [] [] acc = acc
inBoth' (x:xs) ys acc = if elem x ys then inBoth' xs ys (x:acc) else inBoth' xs ys (acc)
...
instance Foldable List where
  foldr f inital Empty = inital
  foldr f inital (LNode a rest ) = f a (foldr f inital rest)
...
instance Foldable TwoList where
  foldr f inital TwoEmpty            = inital
  foldr f inital (TwoNode a aa rest) = f a (f aa (foldr f inital rest))
