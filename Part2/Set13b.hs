...
visit [] _ = return ()
visit graph place = do
    visited <- get
    if place `elem` visited
        then return ()
        else do
            modify (place :)
            let neighbors = concat [ns | (p, ns) <- graph, p == place]
            mapM_ (visit graph) neighbors

-- Now you should be able to implement path using visit. If you run
-- visit on a placetodo using an empty state, you'll get a state that
...
path maze place1 place2 =
    let visitedPlaces = execState (visit maze place1) []
    in place2 `elem` visitedPlaces
...
findSum2 ks ns = do 
                ...
                guard (sum `elem` ns)
                return (i,j,sum)
-- findSum2 ks ns = [(i,j, i+j) | j <- ks , i <- ks, (i + j) `elem` ns]
...
instance Functor SL where
  -- implement fmap
  fmap :: (a -> b) -> SL a -> SL b
  fmap f (SL a) = SL (\i -> let (x, newState, log) = a i
                            in (f x, newState, log))
...
instance Monad SL where
  ...
  (>>=) :: SL a -> (a -> SL b) -> SL b
  (SL g) >>= f = SL (\i -> 
    let (x, newState, log1) = g i
        SL h = f x
        (y, finalState, log2) = h newState
    in (y, finalState, log1 ++ log2))
