...
workload nExercises hoursPerExercise
                                    | totalWork > 100 = "Holy moly!"
                                    | totalWork < 10  = "Piece of cake!"
                                    | otherwise       = "Ok."
                                  where totalWork = nExercises * hoursPerExercise
...
countValid [] = 0
countValid ss
            | length (head ss) < 6 = countValid (tail ss)
            | (!!) (head ss) 2 /= (!!) (head ss) 4 && (!!) (head ss) 3 /= (!!) (head ss) 5 = countValid (tail ss)
            | otherwise = 1 + countValid (tail ss)
...
repeated [] = Nothing
repeated ele = repeated' (tail ele) (head ele)

repeated' [] _ = Nothing
repeated' list candid = if head list == candid then Just candid else repeated' (tail list) (head list)
...
sumSuccess list = sumSuccess' (tail list) (head list)

sumSuccess' [] (Left _)  = Left "no data"
sumSuccess' [] (Right r) = Right r

sumSuccess' (Left _ :xs )  (Left _  )  = sumSuccess' xs (Left "")
sumSuccess' (Left _ :xs )  (Right r )  = sumSuccess' xs (Right r)
sumSuccess' (Right r :xs)  (Left _  )  = sumSuccess' xs (Right r)
sumSuccess' (Right r :xs)  (Right r2)  = sumSuccess' xs (Right (r + r2))
...
open s (Locked l) = if s == l then Opened l else Locked l
open s (Opened l) = Opened l
...
changeCode s (Locked l) = Locked l
changeCode s (Opened l) = Opened s
...
instance Eq Text where
  (==) (Text s1 ) (Text s2 ) = let newS1 = filter (not . isSpace) s1
                                   newS2 = filter (not . isSpace) s2
                               in newS1 == newS2
------------------------------------------------------------------------------
...
permute indices s = permute' indices s s

permute':: Permutation -> [a] ->  [a] -> [a]
permute' [] _ acc = acc 
permute' (i:ii) (s:ss) acc = permute' ii ss (replace s i acc) 

replace :: a -> Int -> [a] -> [a]
replace chr point acc = let (first, s:ss) = splitAt point acc
                        in first ++ chr:ss
