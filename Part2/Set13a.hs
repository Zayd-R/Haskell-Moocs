...
split s = let firstHalf = takeWhile (/= ' ') s
          in if length firstHalf == length s then Nothing else Just (splitAt (length firstHalf) s)
...
checkNumber (s1, s2) = if checkString s1 && checkString s2 then Just (s1,s2) else Nothing

checkString xs = foldr ((&&) . not . isDigit) True xs
...
checkCapitals (x:xs, y:e:ys) = if isUpper x && isUpper e then Just (x:xs, e:ys) else Nothing
...
winner scores player1 player2 = let score1 = lookup player1 scores
                                    score2 = lookup player2 scores
                                in  combines (score1,score2) ?>  result player1 player2

combines (Nothing, _) = Nothing
combines (_, Nothing) = Nothing
combines (Just x, Just y) = Just (x,y)

result  s1 s2 (a,b)  = if a >= b then Just s1 else Just s2
...
selectSum _ [] = Just 0
selectSum [] _ = Nothing
selectSum xs (i:is) =  selectSum' xs (i:is) 0

selectSum' :: Num b => [b] -> [Int] -> b -> Maybe b
selectSum' _ [] acc = Just acc
selectSum' xs (i:is) acc = safeIndex xs i >>= mySum acc >>=  selectSum' xs is

mySum :: Num a => a -> a -> Maybe a
mySum acc n = Just (acc + n)
...
countAndLog f xs = let (strings, number) = countLog f xs ([],0)
                   in Logger strings number

countLog f [] acc = acc
countLog f (x:xs) acc = if f x then countLog f xs (fst acc ++ [show x], snd acc + 1) else countLog f xs acc
...
rob from to = let BankOp f1 =  balance from
                  BankOp f2 = balance to 
              in BankOp f1 +> takeInt from +> depositOp to


takeInt :: String -> Int -> BankOp Int
takeInt s n = withdrawOp s n
------------------------------------------------------------------------------
...
paren c = if c == '(' then modify (\s -> if s < 0 then s else s + 1) else  modify (\s -> if s < 0 then s else s -1)
...
count x = modify(\s -> case lookup x s of 
                      Nothing -> s ++ [(x,1)]
                      Just val -> filter (\(a, _) -> a /= x) s ++ [(x, val + 1)] )
