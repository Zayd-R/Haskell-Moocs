...
type Size = Int
--   prettyPrint 3 [(1,1),(2,3),(3,2)] ==> "Q..\n..Q\n.Q.\n"
--   prettyPrint 4 [(4,4),(1,4),(1,1),(4,1)]
prettyPrint :: Size -> [Coord] -> String
prettyPrint size coords = prettyPrint' size (1,1) (sort coords)  ""


prettyPrint' :: Col -> Coord -> [(Row, Col)] -> [Char] -> [Char]
prettyPrint' size (row,col) coords acc
                | ((r,c):rest)<- coords, (row,col) == (r,c) = prettyPrint' size (nextCol (row,col)) rest (acc ++ "Q")
                | col <= size                               = prettyPrint' size (nextCol (row,col)) coords (acc ++ ".")
                | row < size && col > size                  = prettyPrint' size (nextRow (row,col)) coords (acc ++ "\n")
                | otherwise                                 = acc ++ "\n"
...
danger _ [] = False
danger (row, col) ((r,c):rest) = sameCol      (row, col) (r, c) ||
                                 sameRow      (row, col) (r, c) ||
                                 sameDiag     (row, col) (r, c) ||
                                 sameAntidiag (row, col) (r, c) ||
                                 danger (row,col) rest
...
prettyPrint2 size coords = prettyPrint2' size (1,1) (sort coords)  ""


prettyPrint2' :: Col -> Coord -> [(Row, Col)] -> [Char] -> [Char]
prettyPrint2' size (row,col) coords acc
                | (row,col) `elem` coords = prettyPrint2' size (nextCol (row,col)) coords (acc ++ "Q")
                | col <= size, not (danger (row,col) coords) = prettyPrint2' size (nextCol (row,col)) coords (acc ++ ".")
                | col <= size, danger (row,col) coords      = prettyPrint2' size (nextCol (row,col)) coords (acc ++ "#")
                | row < size && col > size                  = prettyPrint2' size (nextRow (row,col)) coords (acc ++ "\n")
                | otherwise                                 = acc ++ "\n"
...
fixFirst n [] = Nothing
fixFirst n (s:ss) = case fixFirst' n s ss of 
                        Nothing -> Nothing
                        Just value -> Just (value : ss)

fixFirst' :: Int -> Coord -> Stack -> Maybe Candidate
fixFirst' size queen [] = Nothing 
fixFirst' size queen ss
                    | danger queen ss, snd queen <= size = fixFirst' size (nextCol queen) ss 
...
backtrack (s:[]) = []
backtrack (s:ss:sss) = nextCol ss : sss
...
step size (s:[]) = continue ([s]) 
step size s = case fixFirst size s of 
                    Nothing    -> backtrack s
