...
appendAll ref (x:xs) = do
                    refString <- readIORef ref
                    writeIORef ref (refString ++ x)
                    appendAll ref xs
...
swapIORefs firstRef sndRef = do
                        firstString <- readIORef firstRef
                        sndString <- readIORef sndRef
                        writeIORef sndRef firstString
                        writeIORef firstRef sndString
...
hSelectLines h (x:xs) = do
                    lines <- hFetchLines h
                    return  (hSelectLines' lines (x:xs))

hSelectLines' :: [String] -> [Int] -> [String]
hSelectLines' lines xs = map (\ x -> (!!) lines (x - 1)) xs
...
interact' f state = do 
                userLine <- getLine 
                let (bool,string,newState) = f (userLine, state )
                putStrLn string 
