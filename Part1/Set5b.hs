...
treeMax Empty = 0
treeMax (Node a tree1 tree2)= max (max a (treeMax tree1)) (treeMax tree2)
...
allValues condition (Node a tree1 tree2) = condition a && allValues condition tree1 && allValues condition tree2
...
cull _ Empty = Empty
cull val (Node a tree1 tree2) = if val /= a then Node a (cull val tree1) (cull val tree2) else Empty
...
isOrdered (Node a tree1 tree2) = allValues (< a) tree1 && allValues (> a) tree2 && isOrdered tree1 && isOrdered tree2
------------------------------------------------------------------------------
...
walk (StepL:xs) (Node a tree1 _) = walk xs tree1
walk (StepR:xs) (Node a _ tree2) = walk xs tree2
...
set _ _ Empty = Empty
set [] val (Node a tree1 tree2) = Node val tree1 tree2
set (StepL:xs) val (Node a tree1 tree2) = Node a (set xs val tree1) tree2
set (StepR:xs) val (Node a tree1 tree2) = Node a tree1 (set xs val tree2)
...
search val (Node a tree1 tree2)
                | val == a = Just []
                | otherwise = if null result then Nothing else Just result
                where result = search' val (Node a tree1 tree2) []

search' val t acc = case  t of 
                        Empty -> []
                        (Node a Empty tree2) -> if a == val then acc else search' val tree2 (acc ++ [StepR])
                        (Node a tree1 Empty) -> if a == val then acc else search' val tree1 (acc ++ [StepL])
                        (Node a tree1 tree2) -> if a == val then acc else decide (search' val tree2 (acc ++ [StepR])) (search' val tree1 (acc ++ [StepL]))
                        where decide pos1 pos2 = if length pos1 >= length pos2 then pos1 else pos2
