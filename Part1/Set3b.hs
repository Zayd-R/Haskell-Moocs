...
buildList start count end = case count of
                            0 -> [end]
                            v -> start : buildList start (count - 1) end 
...
sums :: Int -> [Int]
sums i = sumsHelper i 1

sumsHelper i current  
                        | i == 0        = []
                        | current  == i = [value] 
                        | otherwise     = value : sumsHelper i (current + 1) 
                    where  value = div (current * (current + 1)) 2
...
indexDefault' xs i def current = case xs of 
                                    []     -> def
                                    x:[]   -> if current == i then x else def 
                                    x: xss -> if current == i then x else indexDefault' xss i def (current + 1)
...
sorted xs = case xs of 
            ...
            x:[] -> True 
            x:xs -> sorted' x xs  
sorted' current xs = case xs of 
                    []   -> True 
                    x:[] -> current <= x
                    x:xs -> current <= x && sorted' x xs 
...
sumsOf :: [Int] -> [Int]
sumsOf xs = sumOf' xs 0 

sumOf' xs current = case xs of 
                        []   -> []
                        x:xs -> current + x : sumOf' xs (current + x )
...
mymaximum bigger initial xs = case xs of
                                []   -> initial
                                x:xs -> if bigger initial x then mymaximum bigger initial xs 
