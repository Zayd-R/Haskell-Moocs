...
firstHalf s = if even size then  take (div size  2) s else take ( div size  2 + 1) s
              where size = length s
...
capitalize s = unwords (helper (words s) [])

helper :: [String] -> [String] -> [String]
helper w acc = case w of
                s:ss ->  helper ss (acc ++ [toUpper (head s) : tail s] )
                [] -> acc
------------------------------------------------------------------------------
...
--
-- Hints:
--   * k^max > max   (NO SHIT SHERLOCK!!)
--   * the function takeWhile

powers :: Int -> Int -> [Int]
powers k max = powersHelper k max [1]

powersHelper :: (Ord a, Num a) => a -> a -> [a] -> [a]
powersHelper k max acc = if candidate <= max then  powersHelper k max (acc ++ [candidate]) else acc
                        where candidate = last acc  * k
...
joinToLength l li = [ candidate | first <- li, last <- li, let candidate = first  ++ last, length candidate == l ]
...
multiCompose fs arg = case fs of  
                        x:xs -> x (multiCompose xs arg )
                        [] -> arg
------------------------------------------------------------------------------
...
multiApp f gs arg = multiAppHelper f gs arg []

multiAppHelper :: ([a] -> t1) -> [t2 -> a] -> t2 -> [a] -> t1
multiAppHelper f gs arg acc = case gs of 
                                g:gTail -> multiAppHelper f gTail arg (acc ++ [g arg])
                                [] -> f acc
...
interpreterHelper commands x y = case commands of 
                                s:ss -> case s of 
                                        "up"     -> interpreterHelper ss x (y + 1)
                                        "down"   -> interpreterHelper ss x ( y - 1)
                                        "right"  -> interpreterHelper ss (x + 1) y
                                        "left"   -> interpreterHelper ss (x - 1) y
                                        "printX" -> show x : interpreterHelper ss x  y
                                        "printY" -> show y : interpreterHelper ss x  y
                                []   ->    []
