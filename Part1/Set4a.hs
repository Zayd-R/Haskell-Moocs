...
allEqual' x xs = case xs of
                    [] -> True
                    x2:xs -> x == x2 && allEqual' x xs
...
distinct li =  length [ first | first <- li, last <- li,  first == last ] == length li
...
rangeOf elements = maximum elements - minimum elements
...
longest :: Ord a => [[a]] -> [a]
longest elements = longest' elements []

longest' :: Ord a => [[a]] -> [a] -> [a]
longest' [] acc = acc
longest' (x:xs) acc
                | length x > length acc = longest' xs x
                | length x < length acc = longest' xs acc
                | otherwise = if head x <= head acc then longest' xs x  else longest' xs acc
...
freqs xs = Map.fromListWith (+) (foldr (\x acc ->  (x, 1) : acc) [] xs)
------------------------------------------------------------------------------
...
transfer from to amount bank
    ...
    | amount < 0 = bank
    | Just v <- Map.lookup from bank, v < amount  = bank
    | Just v1 <- Map.lookup from bank, Just v2 <- Map.lookup to bank = Map.insert to (v2 + amount) (Map.insert from (v1 - amount) bank) -- a use case of monad ???
...
maxIndex arr = fst (foldl (\acc y -> if snd acc >= snd y then acc else y) initial new_arr )
                where new_arr = assocs arr
                      initial = head new_arr
