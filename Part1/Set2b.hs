...
leftpad s x = if size >= x then s else spaces (x - size) ++ s
            where size = length s
                  spaces 0 = ""
                  spaces x =  " " ++ spaces (x - 1)
...
countdown count = countdownHelper count "Ready! "
countdownHelper 0 str = str ++ " Liftoff!"
countdownHelper count str = countdownHelper (count - 1) (str ++ show count ++ "... ")
...
smallestDivisorHelper x acc = if x `mod`  acc == 0 then acc else smallestDivisorHelper x (acc + 1)
...
biggestPrimeAtMost x
  | x < 2 = error "No prime numbers less than 2"
  | isPrime x = x
