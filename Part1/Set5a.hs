...
study s
      | Freshman  <- s        = NthYear 1
      | NthYear i <- s, i < 7 = NthYear (i + 1)
      | otherwise             = Graduated
...
rgb (Mix color1 color2) =  map
                              (\pair -> (/) (fst pair + snd pair) 2.0 )
                                  (zip (rgb color1) (rgb color2))
...
-- fromList xs = foldr (\x acc -> Pair (fst x) (snd x) acc) Empty xs  -- Another approach
...
prettyPrint' (I End) acc = "1" ++ acc
prettyPrint' (O End) acc = "0" ++ acc
prettyPrint' (I n)   acc = prettyPrint' n ( "1" ++ acc )
prettyPrint' (O n)   acc = prettyPrint' n ( "0" ++ acc )



fromBin :: Bin -> Int
fromBin bin = fromBin' binS 0 index
              where binS = prettyPrint bin
                    index = length binS - 1

fromBin' :: String -> Int -> Int -> Int
fromBin' binS acc index = case binS of
                      ""   -> acc
                      x:xs -> fromBin' xs ( (2^index * digitToInt x) + acc) (index  - 1)


toBin :: Int -> Bin
toBin n = toBin' n (floor(logBase 2 (fromIntegral n))) End

toBin' :: Int -> Int -> Bin -> Bin
toBin' n base acc
          | n == 0, base < 0  = acc
          | n == 0, base >= 0 = toBin' n (base - 1) (O acc)
          | n >= 2^ myLog , myLog /= base = toBin' n (base - 1) (O acc)
          | n >= 2^ myLog , myLog == base = toBin' (n - 2^ myLog) (base - 1) (I acc)
          where myLog = floor(logBase 2 (fromIntegral n) )
