...
add ele (Set [])    = Set [ele]
add ele (Set xs) = if member ele (Set xs) then Set xs else Set ( sort (xs ++ [ele]) )
...
data State = Start | Error | Finished |Egged | Floured | Sugared | FlouredAndSugared| Mixed
  deriving (Eq,Show)
...
instance Semigroup (Set a) => Monoid (Set a) where 
  mempty = Set []
  mappend = (<>)
...
passwordAllowed [] (ContainsSome sub) = False
passwordAllowed (x:xs) (ContainsSome sub) = x `elem` sub ||  passwordAllowed xs (ContainsSome sub)

passwordAllowed password (DoesNotContain sub) = not (passwordAllowed password (ContainsSome sub))
...
operation op (Literal x) (Literal y) = Operation  op (Literal x) (Literal y)
operation op arith1 arith2 = Operation op arith1 arith2
...
-- render (Operation op arith1 arith2) = "(" ++ render arith1 ++ op ++ render arith2  ++ ")"
render (Operation op arith1 arith2) = "(" ++ render arith1  ++ op ++ render arith2 ++ ")"