...
countHelper maybe x = case maybe of
                        Just _  -> x
                        Nothing -> x + 1
...
slHelper ele acc = (ele + fst acc, snd acc + 1 )
...
concatHelper :: [a] -> [a] -> [a]
concatHelper liEle acc = liEle ++ acc 
...
largestHelper ele acc = case acc of 
                        []   -> [ele]
                        x:xs -> if x > ele then acc else 
                                                    if x == ele then ele : acc else [ele] 
...
lastHelper ele acc = case acc of 
                        Nothing -> Just ele 
                        _ -> acc
