...
instance Ord Country where
  ...
  compare count1 count2 = case count1 of
                          Norway -> case count2 of
                                          Switzerland -> LT
                                          Norway -> EQ
                                          Finland -> GT
                          Switzerland -> case count2 of
                                            Switzerland -> EQ
                                            _ -> GT
                          Finland  -> case count2 of
                                      Finland -> EQ
                                      _ -> LT
...
-- implement me?
  ...
  max x y = if x <= y then y else x -- and me?
...
instance Ord Number where
  compare num1 num2
              | Finite _  <- num1, Infinite  <- num2 = LT
              | Infinite  <- num1, Finite _  <- num2 = GT
              | Infinite  <- num1, Infinite  <- num2 = EQ
              | Finite n1 <- num1, Finite n2 <- num2 = compare n1 n2
...
simplify (RationalNumber a b) = let common = myGcd (abs a) b in
                                  RationalNumber (div a common) (div b common)
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b) = let first
                                      | a < 0     = -1
                                      | a > 0     = 1
                                      | otherwise = 0
                                    second
  ...
  negate (RationalNumber x b) = RationalNumber (negate x) b
