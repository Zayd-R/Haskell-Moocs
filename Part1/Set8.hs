...
blendColor (Color x1 y1 c1) (Color x2 y2 c2) = Color (f x1 x2) (f y1 y2) (f c1 c2)
          where f c1 c2 = div (c1 + c2) 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine func  (Picture p1) (Picture p2) =   Picture (\coord -> func (p1 coord) (p2 coord))
------------------------------------------------------------------------------
...
rectangle x y w h = Shape f 
                    where f (Coord px py) =  px >= x && px < x + w && py >= y && py < y + h
------------------------------------------------------------------------------
...
cut (Shape s1) (Shape s2)  = Shape(\coord -> if s2 coord then not (s1 coord) else s1 coord)
------------------------------------------------------------------------------
...
paintSolid color (Shape s1) (Picture p1) = Picture(\coord -> if s1 coord then color else p1 coord)
------------------------------------------------------------------------------
...
paint (Picture p1) (Shape s1) (Picture pb) = Picture(\coord -> if s1 coord then p1 coord else pb coord )
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill c) (Picture p1) = Picture(\coord -> c ) 

data Zoom = Zoom Int
  deriving Show

instance Transform Zoom where
  apply (Zoom n) (Picture p1) = Picture (p1 . zoomCoord n)
...
instance Transform Flip where
  apply (FlipXY) (Picture p1) = Picture(p1 . flipCoordXY)
  apply (FlipX) (Picture p1) = Picture(p1 . flipCoordX)
  apply (FlipY) (Picture p1) = Picture(p1 . flipCoordY)
...
neighbors (Coord x y) = [Coord (x + 1) y, Coord (x - 1) y, Coord x (y -1), Coord x (y + 1)] 

averageColor colors = 
    let (r, g, b, count) = foldr (\(Color r g b) (rt, gt, bt, c) -> (rt + r, gt + g, bt + b, c + 1)) (0, 0, 0, 0) colors
    in Color (r `div` count) (g `div` count) (b `div` count)


-- Function to get the color at a specific coordinate
getColorAt :: Picture -> Coord -> Color
getColorAt (Picture f) coord = f coord


instance Transform Blur where
  apply (Blur) p = Picture (\coord -> 
                    let neighborCoords = coord : neighbors coord
                        neighborColors = map (getColorAt p) neighborCoords
                    in averageColor neighborColors
                    )
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  ...
  apply (BlurMany n) p = apply (BlurMany (n -1))  
------------------------------------------------------------------------------
